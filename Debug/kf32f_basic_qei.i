...
# 15 "D:\\workspace32\\KF32Fxxx_StdPeriph_Lib_V2.3\\inc/kf32f_basic_qei.h" 2
...
void PCLK_CTL3_Peripheral_Clock_Enable (uint32_t PCLK_CTL3_bit,
                    FunctionalState NewState);
# 18 "../src/kf32f_basic_qei.c" 2
# 43 "../src/kf32f_basic_qei.c"
void
QEI_Reset (QEI_SFRmap* QEIx)
{
    ...
    if (QEIx == ((QEI_SFRmap *) ((uint32_t)0x40000500)))
    {
    ...
    if (QEIx == ((QEI_SFRmap *) ((uint32_t)0x40000080)))
    {
...
QEI_Configuration(QEI_SFRmap* QEIx, QEI_InitTypeDef* qeiInitStruct)
{
    ...
    tmpreg = (qeiInitStruct->m_WorkClock)
           | (qeiInitStruct->m_DigitalFilterEn << (4))
           | (qeiInitStruct->m_DigitalFilterPrescaler)
           | (qeiInitStruct->m_SwapABEn << (0));
    QEIx->CTL1 = SFR_Config (QEIx->CTL1,
                          ~(((uint32_t)3<<((5))) | ((uint32_t)1<<((4))) | ((uint32_t)7<<((1))) | ((uint32_t)1<<((0)))),
                          tmpreg);




    tmpreg = (qeiInitStruct->m_IndexReset << (4))
           | (qeiInitStruct->m_DirectionEn << (1));
    QEIx->CTL0 = SFR_Config (QEIx->CTL0,
                          ~(((uint32_t)1<<((4))) | ((uint32_t)1<<((1)))),
                          tmpreg);
}
...
QEI_Struct_Init (QEI_InitTypeDef* qeiInitStruct)
{
    ...
    qeiInitStruct->m_WorkClock = ((uint32_t)0<<(5));

    qeiInitStruct->m_DigitalFilterEn = FALSE;

    qeiInitStruct->m_DigitalFilterPrescaler = ((uint32_t)0<<(1));
...
QEI_Timer_Struct_Init (QEI_TimerTypeDef* qeiTimerInitStruct)
{
    ...
    qeiTimerInitStruct->m_CounterMode = ((uint32_t)0<<(0));

    qeiTimerInitStruct->m_ClockSource = ((uint32_t)0<<(3));
}
# 222 "../src/kf32f_basic_qei.c"
void
QEI_Encoder_Mode_Config (QEI_SFRmap* QEIx, uint32_t PortMode)
{
    ...
    tmpreg = PortMode << (5);
    QEIx->CTL0 = SFR_Config (QEIx->CTL0, ~((uint32_t)7<<((5))), tmpreg);
}
...
QEI_Location_Counter_Reset_Enable (QEI_SFRmap* QEIx, FunctionalState NewState)
{
    ...
    if (NewState != FALSE)
    {
        ...
        __asm volatile("SET [%0], #%1"::"r"(&(QEIx->CTL0)),"i"((4)));
    }
    else
    {

        __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->CTL0)),"i"((4)));
    }
}
# 270 "../src/kf32f_basic_qei.c"
void
QEI_Timer_Source_Clock_Config (QEI_SFRmap* QEIx, uint32_t TimerClock)
{
    ...
    if (TimerClock != ((uint32_t)0<<(3)))
    {

        __asm volatile("SET [%0], #%1"::"r"(&(QEIx->CTL0)),"i"((3)));
    }
    else
    {

        __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->CTL0)),"i"((3)));
    }
}
...
QEI_Location_Counter_Output_Enable (QEI_SFRmap* QEIx, FunctionalState NewState)
{
    ...
    if (NewState != FALSE)
    {
        ...
        __asm volatile("SET [%0], #%1"::"r"(&(QEIx->CTL0)),"i"((1)));
    }
    else
    {

        __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->CTL0)),"i"((1)));
    }
}
# 348 "../src/kf32f_basic_qei.c"
void
QEI_Location_Counter_Direction_Config (QEI_SFRmap* QEIx, uint32_t Direction)
{
    ...
    if (Direction != ((uint32_t)0<<(0)))
    {

        __asm volatile("SET [%0], #%1"::"r"(&(QEIx->CTL0)),"i"((0)));
    }
    else
    {

        __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->CTL0)),"i"((0)));
    }
}
...
QEI_Digital_Filter_Output_Enable (QEI_SFRmap* QEIx, FunctionalState NewState)
{
    ...
    if (NewState != FALSE)
    {
        ...
        __asm volatile("SET [%0], #%1"::"r"(&(QEIx->CTL1)),"i"((4)));
    }
    else
    {

        __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->CTL1)),"i"((4)));
    }
}
# 455 "../src/kf32f_basic_qei.c"
void
...
QEI_Swap_Phase_AB_Enable (QEI_SFRmap* QEIx, FunctionalState NewState)
{
    ...
    if (NewState != FALSE)
    {
        ...
        __asm volatile("SET [%0], #%1"::"r"(&(QEIx->CTL1)),"i"((0)));
    }
    else
    {

        __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->CTL1)),"i"((0)));
    }
}
...
QEI_Clear_INT_Flag (QEI_SFRmap* QEIx, uint32_t InterruptType)
{
    ...
    tmpreg1 = InterruptType << (8);
    tmpreg = InterruptType << (3);
    QEIx->DIER |= tmpreg;
    while(QEIx->CTL0 & tmpreg1);
    QEIx->DIER &= ~tmpreg;
}
# 667 "../src/kf32f_basic_qei.c"
void
...
QEI_Clear_Timer_Overflow_INT_Flag (QEI_SFRmap* QEIx)
{
    ...
    __asm volatile("SET [%0], #%1"::"r"(&(QEIx->DIER)),"i"((5)));
    while(QEIx->CTL0 & ((uint32_t)1<<((10))));
    __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->DIER)),"i"((5)));
}
...
QEI_Clear_Phase_Error_INT_Flag (QEI_SFRmap* QEIx)
{
    ...
    __asm volatile("SET [%0], #%1"::"r"(&(QEIx->DIER)),"i"((4)));
    while(QEIx->CTL0 & ((uint32_t)1<<((9))));
    __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->DIER)),"i"((4)));
}
...
QEI_Clear_Counter_Error_INT_Flag (QEI_SFRmap* QEIx)
{
    ...
    __asm volatile("SET [%0], #%1"::"r"(&(QEIx->DIER)),"i"((3)));
    while(QEIx->CTL0 & ((uint32_t)1<<((8))));
    __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->DIER)),"i"((3)));
}
...
QEI_Timer_Overflow_INT_Enable (QEI_SFRmap* QEIx, FunctionalState NewState)
{
    ...
    if (NewState != FALSE)
    {
        ...
        __asm volatile("SET [%0], #%1"::"r"(&(QEIx->DIER)),"i"((2)));
    }
    else
    {

        __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->DIER)),"i"((2)));
    }
}
...
QEI_Phase_Error_INT_Enable (QEI_SFRmap* QEIx, FunctionalState NewState)
{
    ...
    if (NewState != FALSE)
    {
        ...
        __asm volatile("SET [%0], #%1"::"r"(&(QEIx->DIER)),"i"((1)));
    }
    else
    {

        __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->DIER)),"i"((1)));
    }
}
...
QEI_Counter_Error_INT_Enable (QEI_SFRmap* QEIx, FunctionalState NewState)
{
    ...
    if (NewState != FALSE)
    {
        ...
        __asm volatile("SET [%0], #%1"::"r"(&(QEIx->DIER)),"i"((0)));
    }
    else
    {

        __asm volatile("CLR [%0], #%1"::"r"(&(QEIx->DIER)),"i"((0)));
    }
}
